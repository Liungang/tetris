...
import { Tetromino, TetrominoType, BlockMatrix } from './Tetromino'

// Game constants
const ROWS = 20
const COLS = 10
let BLOCK_SIZE = 30  // default block size
const COLORS = [
  0xFF0000, // Red
  0x00FF00, // Green
  0x0000FF, // Blue
...
export class TetrisGame {
  ...
  constructor(app: CustomPixiApp, nextPieceApp: CustomPixiApp, callbacks: GameCallbacks, blockSize?: number) {
    this.app = app
  ...
  generatePiece(): void {
    ...
    if (!this.nextPiece) {
      this.nextPiece = new Tetromino(this.getRandomPieceType(), COLORS[Math.floor(Math.random() * COLORS.length)])
    }
    ...
    if (this.currentPiece) {
      this.currentPiece.x = Math.floor(COLS / 2) - Math.floor(this.currentPiece.blocks[0].length / 2)
      ...
      for (let row = 0; row < this.currentPiece.blocks.length; row++) {
        for (let col = 0; col < this.currentPiece.blocks[row].length; col++) {
          if (this.currentPiece.blocks[row][col]) {
            ...
            // Fill
            block.fill({ color: this.currentPiece.color })
            block.rect(0, 0, this.blockSize, this.blockSize)
            
            // Border
            block.stroke({ width: 2, color: 0x000000 })
            block.rect(0, 0, this.blockSize, this.blockSize)
            
            block.x = (this.currentPiece.x + col) * this.blockSize
            block.y = (this.currentPiece.y + row) * this.blockSize
            
            this.activeContainer.addChild(block)
          }
        }
      }
    }
  ...
  drawNextPiece(): void {
    ...
    if (this.nextPiece) {
      // Get the renderer dimensions from the renderer itself
      ...
      const offsetX = (rendererWidth - this.nextPiece.blocks[0].length * this.blockSize) / 2
      const offsetY = (rendererHeight - this.nextPiece.blocks.length * this.blockSize) / 2
      ...
      for (let row = 0; row < this.nextPiece.blocks.length; row++) {
        for (let col = 0; col < this.nextPiece.blocks[row].length; col++) {
          if (this.nextPiece.blocks[row][col]) {
            ...
            // Fill
            block.fill({ color: this.nextPiece.color })
            block.rect(0, 0, this.blockSize, this.blockSize)
            ...
            block.x = offsetX + col * this.blockSize
            block.y = offsetY + row * this.blockSize
  ...
  updateGhostPiece(): void {
    ...
    if (this.currentPiece) {
      ...
      while (!this.checkCollision(this.ghostPiece.blocks, this.ghostPiece.x, this.ghostPiece.y + 1)) {
        this.ghostPiece.y++
      }
      
      // Only draw ghost if it's in a different position
        ...
        for (let row = 0; row < this.ghostPiece.blocks.length; row++) {
          for (let col = 0; col < this.ghostPiece.blocks[row].length; col++) {
            if (this.ghostPiece.blocks[row][col]) {
              ...
              block.fill({ color: this.ghostPiece.color, alpha: 0.3 })
              block.rect(0, 0, this.blockSize, this.blockSize)
              
              // Border with transparency
              block.stroke({ width: 2, color: 0x000000, alpha: 0.3 })
              block.rect(0, 0, this.blockSize, this.blockSize)
              
              block.x = (this.ghostPiece.x + col) * this.blockSize
              block.y = (this.ghostPiece.y + row) * this.blockSize
    ...
    for (let row = 0; row < ROWS; row++) {
      for (let col = 0; col < COLS; col++) {
        if (this.board[row][col] !== 0) {
          ...
          // Fill - use the direct color value instead of treating it as an index
          block.fill({ color: this.board[row][col] })
          block.rect(0, 0, this.blockSize, this.blockSize)
  ...
  checkCollision(blocks: BlockMatrix = this.currentPiece?.blocks || [], 
                x: number = this.currentPiece?.x || 0, 
                y: number = this.currentPiece?.y || 0): boolean {
  ...
  moveLeft(): void {
    if (!this.isRunning || this.isPaused || this.gameOver || !this.currentPiece) return
    
    const newX = this.currentPiece.x - 1
    if (!this.checkCollision(this.currentPiece.blocks, newX, this.currentPiece.y)) {
  ...
  moveDown(): boolean {
    if (!this.isRunning || this.isPaused || this.gameOver || !this.currentPiece) return false
    
    const newY = this.currentPiece.y + 1
    if (!this.checkCollision(this.currentPiece.blocks, this.currentPiece.x, newY)) {
  ...
  rotateMatrix(matrix: BlockMatrix): BlockMatrix {
    ...
    const result: BlockMatrix = Array(N).fill(0).map(() => Array(N).fill(0))
